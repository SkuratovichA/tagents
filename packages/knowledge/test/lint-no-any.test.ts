...
//
// The same gate @tagents/core runs, pointed at this package. It is a copy and
// stays a copy on purpose: a shared helper would have to be imported from a
// built dist, and a lint that only works after a build is a lint that does not
// run when it matters.
//
...
test('src and test carry TypeScript files to lint at all', () => {
  ...
  assert.ok(files.length >= 10, `expected the modules and their suites, found ${files.length}`);
});
